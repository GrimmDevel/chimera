...
#pragma once
#ifndef CHIMERA_IPC_MESSAGE_H
#define CHIMERA_IPC_MESSAGE_H
...
// header
typedef struct CHIMERA_PACKED mach_msg_header {
...
CHIMERA_STATIC_ASSERT(sizeof(mach_msg_header_t) == MACH_MSG_HEADER_SIZE,
...
typedef struct CHIMERA_PACKED mach_msg_type_descriptor {
    u32                         pad1;
    u32                         pad2;
    u16                         pad3;
    u8                          pad4;
...
typedef struct CHIMERA_PACKED mach_msg_port_descriptor {
...
typedef struct CHIMERA_PACKED mach_msg_ool_descriptor {
    chimera_vaddr_t                 address;
    u8                          deallocate;
    u8                          copy;
    u8                          pad1;
    mach_msg_descriptor_type_t  type;
...
CHIMERA_STATIC_ASSERT(sizeof(mach_msg_type_descriptor_t) == 12, "type desc size 12");
CHIMERA_STATIC_ASSERT(sizeof(mach_msg_port_descriptor_t) == 12, "port desc size 12");
CHIMERA_STATIC_ASSERT(sizeof(mach_msg_ool_descriptor_t) == 16, "ool desc size 16");

typedef u32 mach_msg_trailer_type_t;
#define MACH_MSG_TRAILER_FORMAT_0   0u

typedef struct CHIMERA_PACKED mach_msg_trailer {
    mach_msg_trailer_type_t msgh_trailer_type;
    mach_msg_size_t         msgh_trailer_size;
} mach_msg_trailer_t;

typedef struct CHIMERA_PACKED mach_msg_audit_trailer {
    mach_msg_trailer_type_t msgh_trailer_type;
    mach_msg_size_t         msgh_trailer_size;
    u32                     msgh_seqno;
    chimera_pid_t               msgh_sender_pid;
    chimera_uid_t               msgh_sender_uid;
...
typedef struct ipc_kmsg {
    ...
    chimera_pid_t               ikm_sender_pid;
    chimera_uid_t               ikm_sender_uid;
    u64                     ikm_send_time;
    ...
    u32                     ikm_ool_count;
    struct {
        chimera_vaddr_t         addr;
        chimera_size_t          size;
    } ikm_ool_regions[16];

    u8 CHIMERA_ALIGNED(64)      ikm_data[];
} ipc_kmsg_t;
...
CHIMERA_WARN_UNUSED
chimera_error_t ipc_kmsg_copyin(ipc_kmsg_t *kmsg,
                             chimera_vaddr_t user_header,
                             ipc_space_t *space);

CHIMERA_WARN_UNUSED
chimera_error_t ipc_kmsg_copyout(ipc_kmsg_t *kmsg,
                              chimera_vaddr_t user_buf,
                              mach_msg_size_t buf_size,
                              ipc_space_t *space);

CHIMERA_WARN_UNUSED
chimera_error_t ipc_mqueue_send(ipc_port_t *port, ipc_kmsg_t *kmsg,
                             mach_msg_timeout_t timeout_ms);

CHIMERA_WARN_UNUSED
chimera_error_t ipc_mqueue_receive(ipc_port_t *port, ipc_kmsg_t **kmsg_out,
                                mach_msg_timeout_t timeout_ms);
