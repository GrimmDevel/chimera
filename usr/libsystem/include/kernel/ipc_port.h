...
CHIMERA_ALWAYS_INLINE bool ipc_port_is_active(const ipc_port_t *port) {
    return port->ip_state == IPC_PORT_STATE_ACTIVE;
}

CHIMERA_ALWAYS_INLINE bool ipc_port_is_dead(const ipc_port_t *port) {
    return port->ip_state == IPC_PORT_STATE_DEAD;
}

CHIMERA_ALWAYS_INLINE u32 ipc_port_msgcount(const ipc_port_t *port) {
