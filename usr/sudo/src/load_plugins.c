...
static int builtin_open(unsigned int version, sudo_conv_t conv, sudo_printf_t pr, char * const settings[], char * const user_info[], char * const user_env[], char * const plugin_options[], const char **errstr) {
    (void)version; (void)conv; (void)pr; (void)settings; (void)user_info; (void)user_env; (void)plugin_options; (void)errstr;
    return 1;
}
static void builtin_close(int exit_status, int error) {
    (void)exit_status; (void)error;
}
static int builtin_show_version(int verbose) {
    (void)verbose;
    printf("Sudo version %s\n", PACKAGE_VERSION);
    return 1;
}
extern char **environ;
static int builtin_check_policy(int argc, char * const argv[], char *env_add[], char **command_info[], char **argv_out[], char **user_env_out[], const char **errstr) {
    (void)env_add; (void)errstr;
    if (argv_out) *argv_out = (char **)argv;
    if (user_env_out) *user_env_out = environ;
    if (command_info) {
        static char *default_info[16];
        static char cmnd_buf[PATH_MAX + 16];
        int idx = 0;
        if (argc > 0 && argv && argv[0]) {
            snprintf(cmnd_buf, sizeof(cmnd_buf), "command=%s", argv[0]);
        } else {
            snprintf(cmnd_buf, sizeof(cmnd_buf), "command=/bin/sh");
        }
        default_info[idx++] = cmnd_buf;
        default_info[idx++] = "runas_uid=0";
        default_info[idx++] = "runas_gid=0";
        default_info[idx++] = "runas_user=root";
        default_info[idx++] = "runas_group=wheel";
        default_info[idx++] = NULL;
        *command_info = default_info;
    }
    return 1;
}

static struct policy_plugin builtin_policy = {
    .type = SUDO_POLICY_PLUGIN,
    .version = SUDO_API_VERSION,
    .open = builtin_open,
    .close = builtin_close,
    .show_version = builtin_show_version,
    .check_policy = builtin_check_policy,
};
...
sudo_load_plugins(void)
{
    ...
    if (policy_plugin.handle == NULL) {
	/* Default policy plugin */
	if (!sudo_load_sudoers_plugin("sudoers_policy", false)) {
            policy_plugin.name = "sudoers_policy";
            policy_plugin.u.policy = &builtin_policy;
            policy_plugin.handle = (void *)1;
            ret = true;
        } else {
            ret = true;
        }
	...
	if (TAILQ_EMPTY(&io_plugins)) {
	    (void)sudo_load_sudoers_plugin("sudoers_io", true);
	}
