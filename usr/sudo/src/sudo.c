...
printf("[SUDO] 1 initprogname done\n");

    /* Crank resource limits to unlimited. */
    unlimit_sudo();
printf("[SUDO] 2 unlimit done\n");

    /* Make sure fds 0-2 are open and do OS-specific initialization. */
    fix_fds();
printf("[SUDO] 3 fix_fds done\n");
    os_init(argc, argv, envp);
printf("[SUDO] 4 os_init done\n");

    setlocale(LC_ALL, "");
    bindtextdomain(PACKAGE_NAME, LOCALEDIR);
    textdomain(PACKAGE_NAME);
printf("[SUDO] 5 locale done\n");
...
printf("[SUDO] 12 conf_read2 done\n");

    /* Fill in user_info with user name, uid, cwd, etc. */
printf("[SUDO] 13 pre get_user_info\n");
