...
x86_64 hybrid Mach/BSD operating system inspired by Apple XNU / Darwin. Boots via the custom UEFI Mach-O loader.
...
- **Userspace & Frameworks**: native x86_64 Mach-O binaries loaded by the Mach-O kernel loader, launchd foundation (`launchd-chimera` PID 1), shells (`zsh`, `sh`), Cocoa/NextStep runtime (`objc_runtime_chimera`, `Foundation`, `AppKit`, `CoreGraphics`, `Onyx2D`), GUI WindowServer with compositor, Dock, SystemUIServer, Filer, and tools (`sectest`, `tcc`, `kilo`, `fastfetch`, `curl`, `ping`, `ifconfig`, `nc`).
...
- mtools (for FAT32 UEFI disk image generation)
- OVMF (for QEMU UEFI firmware)
...
# optionally make a persistent data disk image
make disk
...
- `kernel/`
  ...
  - `bsd/` - BSD syscall dispatch, process tables, signal delivery, credentials, Mach-O loader
...
- `scripts/` - QEMU runner (`run_qemu.sh`) and UEFI disk creation tools
- `build/` - CMake build artifacts and `disk.img`
...
# connect gdb
gdb build/x86_64/kernel/mach_kernel -ex "target remote :1234"
```
