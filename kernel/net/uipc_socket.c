/* =============================================================================
 * Chimera Operating System — BSD Socket Layer (uipc_socket)
...
extern chimera_error_t udp_attach(socket_t *so);
extern void udp_detach(socket_t *so);
extern chimera_error_t udp_bind(socket_t *so, struct sockaddr_in *sin);
extern chimera_error_t udp_sendto(socket_t *so, const void *buf, usize len, struct sockaddr_in *dest);

extern chimera_error_t tcp_attach(socket_t *so);
extern void tcp_detach(socket_t *so);
extern chimera_error_t tcp_bind(socket_t *so, struct sockaddr_in *sin);
extern chimera_error_t tcp_connect(socket_t *so, struct sockaddr_in *sin);
extern chimera_error_t tcp_send(socket_t *so, const void *buf, usize len, int flags);
extern chimera_error_t tcp_close(socket_t *so);

chimera_error_t socreate(int dom, socket_t **aso, int type, int proto) {
    if (!aso) return CHIMERA_ERR_INVALID;
    if (dom != AF_INET && dom != AF_UNIX) return CHIMERA_ERR_NOT_SUPPORTED;
...
chimera_error_t sobind(socket_t *so, struct sockaddr *nam) {
    if (!so || !nam) return CHIMERA_ERR_INVALID;
    if (nam->sa_family != AF_INET) return CHIMERA_ERR_NOT_SUPPORTED;
...
chimera_error_t sosend(socket_t *so, struct sockaddr *addr, const void *buf, usize len, int flags) {
    if (!so || !buf) return CHIMERA_ERR_INVALID;

    if (so->so_type == SOCK_DGRAM) {
        if (!addr) return CHIMERA_ERR_INVALID;
...
chimera_error_t soreceive(socket_t *so, struct sockaddr **addr, void *buf, usize len, usize *out_len, int flags) {
    (void)flags;
    (void)addr;
    if (!so || !buf || !out_len) return CHIMERA_ERR_INVALID;
    ...
    if (!so->so_rcv.sb_mb) {
        ...
        *out_len = 0;
        return (so->so_state & SS_CANTRCVMORE) ? CHIMERA_SUCCESS : CHIMERA_ERR_WOULDBLOCK;
    }
...
chimera_error_t soclose(socket_t *so) {
    if (!so || so->so_signature != CHIMERA_SOCKET_MAGIC) return CHIMERA_ERR_INVALID;
...
chimera_error_t sosetopt(socket_t *so, int level, int optname, const void *optval, usize optlen) {
    if (!so || so->so_signature != CHIMERA_SOCKET_MAGIC) return CHIMERA_ERR_INVALID;
    if (!optval || optlen == 0) return CHIMERA_ERR_INVALID;
    ...
    if (level == SOL_SOCKET) {
        ...
        case SO_BROADCAST:
        case SO_DEBUG: {
            if (optlen < sizeof(int)) return CHIMERA_ERR_INVALID;
        ...
        case SO_RCVTIMEO: {
            if (optlen < sizeof(u32)) return CHIMERA_ERR_INVALID;
...
chimera_error_t sogetopt(socket_t *so, int level, int optname, void *optval, usize *optlen) {
    if (!so || so->so_signature != CHIMERA_SOCKET_MAGIC) return CHIMERA_ERR_INVALID;
    if (!optval || !optlen || *optlen == 0) return CHIMERA_ERR_INVALID;
...
i16 sopoll(socket_t *so, i16 events) {
    if (!so || so->so_signature != CHIMERA_SOCKET_MAGIC) return 0x0020; // POLLNVAL
    i16 revents = 0;
