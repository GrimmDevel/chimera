/* =============================================================================
 * Chimera Operating System — File Descriptor Table Implementation
...
static chimera_fileproc_t  s_fp_pool[FILEPROC_POOL_SIZE];
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
chimera_fileproc_t *fp_alloc(vnode_t *vp, u32 flags) {
    CHIMERA_ASSERT(vp != nullptr);

    irq_flags_t irq = spinlock_lock_irqsave(&s_fp_pool_lock);

    chimera_fileproc_t *fp = nullptr;
    ...
    bool dynamic = false;
    if (!fp) {
        fp = (chimera_fileproc_t *)kalloc(sizeof(chimera_fileproc_t));
        if (!fp) {
    ...
    fp->fp_signature = CHIMERA_FILEPROC_MAGIC;
...
void fp_release(chimera_fileproc_t *fp) {
    if (!fp) return;
    CHIMERA_ASSERT(fp->fp_signature == CHIMERA_FILEPROC_MAGIC);
    ...
    if (fp->fp_type == DTYPE_SOCKET && fp->fp_socket) {
        extern chimera_error_t soclose(struct socket *so);
 ...
 *
 * Returns -1 if the FDT is full (CHIMERA_PROC_MAX_FDS entries).
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
int proc_fd_alloc_from(chimera_proc_t *p, chimera_fileproc_t *fp, int min_fd) {
    CHIMERA_ASSERT(p  != nullptr);
    CHIMERA_ASSERT(fp != nullptr);
    if (min_fd < 0) min_fd = 0;
    if (min_fd >= CHIMERA_PROC_MAX_FDS) return -1;

    irq_flags_t irq = spinlock_lock_irqsave(&p->p_fdlock);

    int fd = -1;
    for (int i = min_fd; i < CHIMERA_PROC_MAX_FDS; i++) {
...
int proc_fd_install(chimera_proc_t *p, chimera_fileproc_t *fp) {
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
chimera_fileproc_t *proc_fd_lookup(chimera_proc_t *p, int fd) {
    if (!p || fd < 0 || fd >= CHIMERA_PROC_MAX_FDS) return nullptr;

    irq_flags_t irq = spinlock_lock_irqsave(&p->p_fdlock);
    chimera_fileproc_t *fp = p->p_fd_table[fd];
    if (fp) fp_retain(fp);
    spinlock_unlock_irqrestore(&p->p_fdlock, irq);
...
u8 proc_fd_get_flags(chimera_proc_t *p, int fd) {
    if (!p || fd < 0 || fd >= CHIMERA_PROC_MAX_FDS) return 0;
    irq_flags_t irq = spinlock_lock_irqsave(&p->p_fdlock);
...
void proc_fd_set_flags(chimera_proc_t *p, int fd, u8 flags) {
    if (!p || fd < 0 || fd >= CHIMERA_PROC_MAX_FDS) return;
    irq_flags_t irq = spinlock_lock_irqsave(&p->p_fdlock);
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
chimera_error_t proc_fd_close(chimera_proc_t *p, int fd) {
    if (!p || fd < 0 || fd >= CHIMERA_PROC_MAX_FDS) return CHIMERA_ERR_INVALID;
...
i16 fileproc_poll(chimera_fileproc_t *fp, i16 events) {
    if (!fp || fp->fp_signature != CHIMERA_FILEPROC_MAGIC) return 0x0020; // POLLNVAL
        ...
        if (vp->v_type == VCHR) {
            if (vp->v_op && vp->v_op->vop_name) {
                ...
                if (__builtin_strcmp(vp->v_op->vop_name, "devfs_mouse") == 0) {
                    extern bool chimerakit_hid_has_mouse(void);
                    i16 rev = 0;
                    if ((events & 0x0001) && chimerakit_hid_has_mouse()) rev |= 0x0001;
                    if (events & 0x0004) rev |= 0x0004;
                    return rev;
                }
                if (__builtin_strcmp(vp->v_op->vop_name, "devfs_kbd") == 0) {
                    extern bool chimerakit_hid_has_kbd(void);
                    i16 rev = 0;
                    if ((events & 0x0001) && chimerakit_hid_has_kbd()) rev |= 0x0001;
                    if (events & 0x0004) rev |= 0x0004;
                    return rev;
                }
