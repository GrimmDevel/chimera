...
typedef struct chimera_boot_info {
  u64 magic;
  u32 version;
  u32 flags;
...
extern chimera_boot_info_t *g_boot_info;
extern size_t chimerakit_hid_read_mouse(chimera_event_t *buf, size_t count);
extern u64 g_fb_phys_addr;

// /dev/null
static chimera_error_t dev_null_read(vnode_t *vp, struct uio *uio, int f,
                                 vfs_context_t *ctx) {
  (void)vp;
  (void)uio;
  (void)f;
...
static chimera_error_t dev_null_write(vnode_t *vp, struct uio *uio, int f,
                                  vfs_context_t *ctx) {
  (void)vp;
  (void)uio;
  (void)f;
...
static vnode_ops_t s_null_ops = {.vop_name = "devfs_null",
                                 .vop_read = dev_null_read,
                                 .vop_write = dev_null_write};

extern i64 console_read(char *dst, usize len);
extern void console_write(const char *buf, usize len);

static chimera_error_t dev_console_read(vnode_t *vp, struct uio *uio, int f,
                                    vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
...
static chimera_error_t dev_console_write(vnode_t *vp, struct uio *uio, int f,
                                     vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  ...
  while (uio->uio_resid > 0) {
    usize to_write =
        uio->uio_resid < sizeof(tmp) ? uio->uio_resid : sizeof(tmp);
...
static chimera_error_t dev_console_ioctl(vnode_t *vp, u64 cmd, chimera_vaddr_t arg,
                                     vfs_context_t *ctx) {
  (void)vp;
  (void)ctx;
  extern chimera_error_t console_ioctl(u64 cmd, chimera_vaddr_t arg);
  return console_ioctl(cmd, arg);
}

static vnode_ops_t s_console_ops = {.vop_name = "devfs_console",
                                    .vop_read = dev_console_read,
                                    .vop_write = dev_console_write,
                                    .vop_ioctl = dev_console_ioctl};

// /dev/fb0
static chimera_error_t dev_fb_mmap(vnode_t *vp, chimera_offset_t off, chimera_size_t sz,
...
static chimera_error_t dev_fb_ioctl(vnode_t *vp, u64 cmd, chimera_vaddr_t arg,
                                vfs_context_t *ctx) {
  (void)vp;
  (void)ctx;
  if (cmd == FBIOGET_INFO) {
    ...
    extern chimera_error_t copyin(const void *uaddr, void *kaddr, usize len);
    if (copyin((const void *)arg, &pan, sizeof(pan)) != CHIMERA_SUCCESS) {
...
static vnode_ops_t s_fb_ops = {
    .vop_name = "devfs_fb", .vop_mmap = dev_fb_mmap, .vop_ioctl = dev_fb_ioctl};

// /dev/serial
static chimera_error_t dev_serial_write(vnode_t *vp, struct uio *uio, int f,
                                        vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  ...
  char buf[256];
  usize resid = uio->uio_resid;
  const char *uptr = (const char *)uio->uio_buf;

  while (resid > 0) {
    usize chunk = resid < sizeof(buf) - 1 ? resid : sizeof(buf) - 1;
    if (copyin(uptr, buf, chunk) != CHIMERA_SUCCESS) {
      return CHIMERA_ERR_INVALID;
    }
    buf[chunk] = '\0';
    serial_puts(buf);
  ...
  uio->uio_buf = (void *)uptr;
  uio->uio_resid = 0;
  return CHIMERA_SUCCESS;
}
static vnode_ops_t s_serial_ops = {.vop_name = "devfs_serial",
                                   .vop_read = nullptr,
                                   .vop_write = dev_serial_write};

// /dev/mouse
static chimera_error_t dev_mouse_read(vnode_t *vp, struct uio *uio, int f,
                                  vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  chimera_event_t events[16];
  size_t count = chimerakit_hid_read_mouse(events, 16);
  if (count == 0)
    return CHIMERA_SUCCESS;
  size_t n = count * sizeof(chimera_event_t);
  if (uio->uio_resid < n)
    n = uio->uio_resid;
  extern chimera_error_t copyout(const void *kaddr, void *uaddr, usize len);
  if (copyout(events, uio->uio_buf, n) == CHIMERA_SUCCESS) {
...
static vnode_ops_t s_mouse_ops = {.vop_name = "devfs_mouse",
                                  .vop_read = dev_mouse_read};

// /dev/kbd
extern size_t chimerakit_hid_read_kbd(chimera_event_t *buf, size_t count);

static chimera_error_t dev_kbd_read(vnode_t *vp, struct uio *uio, int f,
                                vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  chimera_event_t events[16];
  size_t count = chimerakit_hid_read_kbd(events, 16);
  if (count == 0)
    return CHIMERA_SUCCESS;
  size_t n = count * sizeof(chimera_event_t);
  if (uio->uio_resid < n)
...
static vnode_ops_t s_kbd_ops = {.vop_name = "devfs_kbd",
                                .vop_read = dev_kbd_read};
...
static chimera_error_t dev_disk_read(vnode_t *vp, struct uio *uio, int f,
                                 vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  ...
  while (uio->uio_resid > 0) {
    if (ata_read_sectors(lba, 1, sec_buf) != CHIMERA_SUCCESS)
      break;
    usize avail = ATA_SECTOR_SIZE - sec_offset;
    usize to_copy = uio->uio_resid < avail ? uio->uio_resid : avail;
    if (copyout(sec_buf + sec_offset, (void *)uio->uio_buf, to_copy) !=
        CHIMERA_SUCCESS)
      return CHIMERA_ERR_GENERIC;
...
static chimera_error_t dev_disk_write(vnode_t *vp, struct uio *uio, int f,
                                  vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  ...
  while (uio->uio_resid > 0) {
    ...
    if (copyin((const void *)uio->uio_buf, sec_buf + sec_offset, to_copy) !=
        CHIMERA_SUCCESS)
      return CHIMERA_ERR_GENERIC;
    if (ata_write_sectors(lba, 1, sec_buf) != CHIMERA_SUCCESS)
      break;
    uio->uio_buf = (void *)((uptr)uio->uio_buf + to_copy);
    uio->uio_resid -= to_copy;
    uio->uio_offset += to_copy;
...
static vnode_ops_t s_disk_ops = {.vop_name = "devfs_disk",
...
static vnode_t s_dev_kbd_vnode;
...
static void devfs_mknode(vnode_t *vp, const char *name, vtype_t type,
                         vnode_ops_t *ops, const char *devpath) {
...
void devfs_init(void) {
  kprintf("        devfs: mounting /dev/null, /dev/serial, /dev/console, "
          "/dev/tty, /dev/ptmx, "
          "/dev/pts/0, /dev/fb0, /dev/mouse, /dev/kbd, /dev/disk0...\n");

  pty_init();

  devfs_mknode(&s_dev_dir_vnode, "dev", VDIR, &s_dev_dir_ops, "/dev");
  devfs_mknode(&s_dev_null_vnode, "null", VCHR, &s_null_ops, "/dev/null");
  devfs_mknode(&s_dev_serial_vnode, "serial", VCHR, &s_serial_ops,
               "/dev/serial");
  devfs_mknode(&s_dev_console_vnode, "console", VCHR, &s_console_ops,
               "/dev/console");
  ...
  devfs_mknode(&s_dev_kbd_vnode, "kbd", VCHR, &s_kbd_ops, "/dev/kbd");
