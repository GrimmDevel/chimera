...
// /dev/null
static xiu_error_t dev_null_read(vnode_t *vp, struct uio *uio, int f,
                                 vfs_context_t *ctx) {
  (void)vp;
  (void)uio;
  (void)f;
...
static xiu_error_t dev_null_write(vnode_t *vp, struct uio *uio, int f,
                                  vfs_context_t *ctx) {
  (void)vp;
  (void)uio;
  (void)f;
...
static xiu_error_t dev_console_read(vnode_t *vp, struct uio *uio, int f,
                                    vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
...
static xiu_error_t dev_console_write(vnode_t *vp, struct uio *uio, int f,
                                     vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
...
static xiu_error_t dev_console_ioctl(vnode_t *vp, u64 cmd, xiu_vaddr_t arg,
                                     vfs_context_t *ctx) {
  (void)vp;
  (void)ctx;
...
static xiu_error_t dev_fb_ioctl(vnode_t *vp, u64 cmd, xiu_vaddr_t arg,
                                vfs_context_t *ctx) {
  (void)vp;
  (void)ctx;
  if (cmd == FBIOGET_INFO) {
...
// /dev/serial
static xiu_error_t dev_serial_write(vnode_t *vp, struct uio *uio, int f,
                                    vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
...
// /dev/mouse
static xiu_error_t dev_mouse_read(vnode_t *vp, struct uio *uio, int f,
                                  vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
...
// /dev/kbd
extern size_t xiukit_hid_read_kbd(xiu_event_t *buf, size_t count);

static xiu_error_t dev_kbd_read(vnode_t *vp, struct uio *uio, int f,
                                vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  xiu_event_t events[16];
  size_t count = xiukit_hid_read_kbd(events, 16);
  if (count == 0)
    return XIU_SUCCESS;
  size_t n = count * sizeof(xiu_event_t);
  if (uio->uio_resid < n)
...
static xiu_error_t dev_disk_read(vnode_t *vp, struct uio *uio, int f,
                                 vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  ...
  while (uio->uio_resid > 0) {
    if (ata_read_sectors(lba, 1, sec_buf) != XIU_SUCCESS)
      break;
...
static xiu_error_t dev_disk_write(vnode_t *vp, struct uio *uio, int f,
                                  vfs_context_t *ctx) {
  (void)vp;
  (void)f;
  (void)ctx;
  ...
  while (uio->uio_resid > 0) {
    ...
    if (copyin((const void *)uio->uio_buf, sec_buf + sec_offset, to_copy) !=
        XIU_SUCCESS)
      return XIU_ERR_GENERIC;
    if (ata_write_sectors(lba, 1, sec_buf) != XIU_SUCCESS)
      break;
    uio->uio_buf = (void *)((uptr)uio->uio_buf + to_copy);
    uio->uio_resid -= to_copy;
    uio->uio_offset += to_copy;
