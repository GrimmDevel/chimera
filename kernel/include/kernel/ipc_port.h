...
#pragma once
#ifndef XIU_IPC_PORT_H
#define XIU_IPC_PORT_H
...
typedef u32 mach_port_type_t;

#define MACH_PORT_TYPE_NONE         ((mach_port_type_t)0x00000000)
#define MACH_PORT_TYPE_SEND         ((mach_port_type_t)0x00000001)
#define MACH_PORT_TYPE_RECEIVE      ((mach_port_type_t)0x00000002)
#define MACH_PORT_TYPE_SEND_ONCE    ((mach_port_type_t)0x00000004)
#define MACH_PORT_TYPE_PORT_SET     ((mach_port_type_t)0x00000008)
#define MACH_PORT_TYPE_DEAD_NAME    ((mach_port_type_t)0x00000010)
#define MACH_PORT_TYPE_SEND_RECEIVE (MACH_PORT_TYPE_SEND | MACH_PORT_TYPE_RECEIVE)

typedef xiu_port_name_t     mach_port_name_t;
typedef u32                 mach_port_seqno_t;

#define MACH_PORT_NAME_NULL     XIU_PORT_NULL
#define MACH_PORT_NAME_DEAD     XIU_PORT_DEAD
...
typedef struct XIU_ALIGNED(64) ipc_port {
    u64                 ip_signature;
    spinlock_t          ip_lock;
...
#define XIU_IPC_PORT_MAGIC  UINT64_C(0x584955504F525421)

typedef struct ipc_entry {
    ipc_port_t         *ie_object;
...
XIU_WARN_UNUSED xiu_error_t ipc_space_create(struct xiu_task *task,
                                              ipc_space_t **space_out);
void                         ipc_space_destroy(ipc_space_t *space);

extern ipc_port_t *ipc_port_kernel_bootstrap;

XIU_ALWAYS_INLINE bool ipc_port_is_active(const ipc_port_t *port) {
    return port->ip_state == IPC_PORT_STATE_ACTIVE;
}

XIU_ALWAYS_INLINE bool ipc_port_is_dead(const ipc_port_t *port) {
    return port->ip_state == IPC_PORT_STATE_DEAD;
}

XIU_ALWAYS_INLINE u32 ipc_port_msgcount(const ipc_port_t *port) {
