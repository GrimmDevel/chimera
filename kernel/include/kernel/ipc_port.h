...
XIU_ALWAYS_INLINE bool ipc_port_is_active(const struct ipc_port *port) {
    return port->ip_state == IPC_PORT_STATE_ACTIVE;
}

XIU_ALWAYS_INLINE bool ipc_port_is_dead(const struct ipc_port *port) {
    return port->ip_state == IPC_PORT_STATE_DEAD;
}

XIU_ALWAYS_INLINE u32 ipc_port_msgcount(const struct ipc_port *port) {
