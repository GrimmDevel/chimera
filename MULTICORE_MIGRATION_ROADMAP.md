...
**Статус:** в работе — LAPIC timer получил отдельный vector `0xE0`; обработчик
подтверждает его только через LAPIC EOI. AP timer остаётся отключённым до
появления per-CPU scheduler.
